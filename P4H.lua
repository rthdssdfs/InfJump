...
function mod:P4H()
	...
	local defaultSettings = '{"PrivateChat":"true","Aim-botCalculateWPing":"true","AntiAFK":"true","ToggleRun":"true","ToggleFly":"true","DisableScriptsOff":"true","AutoKickFlinger":"true","ESPGunDrop":"true","AntiFling":"true","Aim-botCalculateWPing":"true","AutoLoadFriendsViewer":"true","Esp":"true","FreeCamSpeed":"2","InfDamage":"true","SeeDeadChat":"true","FlySpeed":"1.25","FOV":"70","CTRLCLICKTP":"true","PressTForDelete":"true","PanicKey":"114"}'
	...
	function isMouseHoverA(uiobject, ignoreTransparentObjects)
	...
	function isMouseHover(uiobject, ignoreTransparentObjects)
		...
		if not isIntersecting(uiobject) then return false end

		for _, descendant in ipairs(getAllTeveXObjs()) do
			if table_find(uiObjectClasses, descendant.ClassName) and
				isIntersecting(descendant) and descendant ~= uiobject and
				checkVisible(descendant) and (descendant.ZIndex > uiobject.ZIndex or descendant:IsDescendantOf(uiobject)) then

				if descendant.BackgroundTransparency < 0.9 or not ignoreTransparentObjects then
					if not getAttribute(descendant, "Ignore") then
						return false
					end
				end
			end
	...
	function onMouseButton1DownA(uiobject, callback)
		table.insert(uiAPIFunctions.onMouseButton1Down, {uiobject, callback})
	end

	function onMouseButton1UpA(uiobject, callback)
		table.insert(uiAPIFunctions.onMouseButton1Up, {uiobject, callback})
	end

	function onMouseButton1Down(uiobject, callback)
		onTouchStart(function(pos)
			if uiobject.Visible and isMouseHover(uiobject, getAttribute(uiobject, "IgnoreTransparentObjects")) then
				if wspace.CurrentCamera.CameraType == Enum.CameraType.Custom then
					variables.oldcameradata30 = {}
					variables.oldcameradata30.CameraType = wspace.CurrentCamera.CameraType
					wspace.CurrentCamera.CameraType = Enum.CameraType.Scriptable
				end
				callback(pos)
			end
	...
	function onMouseButton1Up(uiobject, callback)
		local function onInputEnded(input)
			if input.UserInputType == Enum.UserInputType.MouseButton1 and
				isMouseHover(uiobject, getAttribute(uiobject, "IgnoreTransparentObjects")) then
				callback()
			end
		end
	...
	function getFg(i)
		return math.floor(i/2) == i/2 and 1 or -1
	end
	...
	function encodetext(text)
		...
		for i = 1, math.huge do
			...
			if string.char(string.byte(text, i)) == " " then
				...
				local fg = getFg(i) -- getFg(math.floor(Random.new(i):NextNumber()+0.5)*9.5)
				encoded = encoded .. string.char(string.byte(text, i) + 1 * fg)
			end
		end
	...
	function decodetext(text)
		...
		for i = 1, math.huge do
			...
			if string.char(string.byte(text, i)) == " " then
				...
				local fg = getFg(i) -- getFg(math.floor(Random.new(i):NextNumber()+0.5)*9.5)
				decoded = decoded .. string.char(string.byte(text, i) - 1 * fg)
			end
		end
	...
	function sendMessage(text)
		if chatService:FindFirstChild("TextChannels") and chatService:FindFirstChild("TextChannels"):FindFirstChild("RBXGeneral") then
			chatService:FindFirstChild("TextChannels"):FindFirstChild("RBXGeneral"):SendAsync(text)
		end
	end

	function sendSafeData(data: string, datatype: string)
		local text = datatype..encodetext(data)
		if not pcall(function() return game:GetService("Players"):Chat(text) end) then
			sendMessage(text)
		end
	end

	function onChat(f)
		table.insert(onChatF,f)
		return {
			Disconnect = function()
				table.remove(onChatF,table_find(onChatF,f))
			end,
		}
	end

	function callOnChat(plr,args)
		for _,v in ipairs(onChatF) do
			task.spawn(v,plr,unpack(args))
		end
	end
	...
	chatService.MessageReceived:Connect(function(textChatMessage)
		if uiClosed or not textChatMessage.TextSource then
			return
		end
	...
	local inforesponser = game:GetService("ReplicatedStorage"):FindFirstChild("Remotes") and game:GetService("ReplicatedStorage"):FindFirstChild("Remotes"):FindFirstChild("Extras") and game:GetService("ReplicatedStorage"):FindFirstChild("Remotes"):FindFirstChild("Extras"):FindFirstChild("GetPlayerData")
	local equipemm2 = game:GetService("ReplicatedStorage"):FindFirstChild("Remotes") and game:GetService("ReplicatedStorage"):FindFirstChild("Remotes"):FindFirstChild("Inventory") and game:GetService("ReplicatedStorage"):FindFirstChild("Remotes"):FindFirstChild("Inventory"):FindFirstChild("Equip")
	...
	icons.Close = "rbxassetid://133542559274293"; -- "rbxassetid://18206051623"; old
	icons.Game = "rbxassetid://18206057575";
	icons.Hide = "rbxassetid://18969516923";
	icons.Minimize = "rbxassetid://102935692399660"; -- "rbxassetid://18206074254"; old
	...
	icons.TapCursor = "rbxassetid://18624189876"; -- "rbxassetid://92958857462028"; new
	icons.TapCursorHover = "rbxassetid://18624371859";
	icons.TapCursorPressed = "rbxassetid://18626630391"; -- "rbxassetid://139862622246237"; -- "rbxassetid://18626630391"; old
	...
	onTouchEnded(function()
		if wspace.CurrentCamera.CameraType == Enum.CameraType.Scriptable and variables.oldcameradata30 then
			wspace.CurrentCamera.CameraType = variables.oldcameradata30.CameraType
		end
	end)
	...
	setFontFace(title, classicFontFaceBold)
	title.Size = UDim2.new(1, -(aeroY*2+12), 1, 0)
	...
	local uiicon = Instance.new("ImageLabel", aero)
	setAttribute(uiicon, "Ignore", true)
	pcall(function() uiicon.Image = icons.Logo; end)
	uiicon.ZIndex = 3
	uiicon.BackgroundTransparency = 1
	uiicon.Position = UDim2.new(0, 2, 0, 0)
	uiicon.Size = UDim2.new(0, aeroY - 4, 1, 0)
	uiicon.Name = "Icon"
	...
	hide.ZIndex = 3
	hide.BackgroundColor3 = Color3.new(0.15,0.15,0.15)
	...
	close.ZIndex = 3
	close.BackgroundColor3 = Color3.new(0.15,0.15,0.15)
		...
		if variables[variablen] then
			...
			if bar then
				...
				local function onMove()
					local newSizeX = (mouse.X - bar.AbsolutePosition.X)
				...
				functions[#functions + 1] = {bar.MouseButton1Down,
					function()
						moving = true
	...
	local characters = {}
	local onJumpRequestf = {}

	function callOnJumpRequest()
		for _, f in ipairs(onJumpRequestf) do
			task.spawn(f)
		end
	end

	function onJumpRequest(f)
		table.insert(onJumpRequestf, f)
		return {
			Disconnect = function()
				table.remove(onJumpRequestf, table.find(onJumpRequestf, f))
			end,
		}
	end

	uis.JumpRequest:Connect(callOnJumpRequest)
	...
	local function fireTrap(cf)
		local trap, trap2 =
			plr.PlayerGui:FindFirstChild("Trap") and plr.PlayerGui:FindFirstChild("Trap"):FindFirstChild("Activate"), game:GetService("ReplicatedStorage"):FindFirstChild("TrapSystem") and game:GetService("ReplicatedStorage"):FindFirstChild("TrapSystem"):FindFirstChild("PlaceTrap")
		if trap then
			trap:FireServer(cf)
		end
		if trap2 then
	...
	local function fireFakeGun(bool)
		local fakegun, fakegun2 =
			plr.PlayerGui:FindFirstChild("FakeGun") and plr.PlayerGui:FindFirstChild("FakeGun"):FindFirstChild("Activate"), game:GetService("ReplicatedStorage"):FindFirstChild("Remotes") and game:GetService("ReplicatedStorage"):FindFirstChild("Remotes"):FindFirstChild("Gameplay") and game:GetService("ReplicatedStorage"):FindFirstChild("Remotes"):FindFirstChild("Gameplay"):FindFirstChild("FakeGun")
		if fakegun then
	...
	local function fireGhost(bool)
		...
		local ghost2 = game:GetService("ReplicatedStorage"):FindFirstChild("Remotes") and game:GetService("ReplicatedStorage"):FindFirstChild("Remotes"):FindFirstChild("Gameplay") and game:GetService("ReplicatedStorage"):FindFirstChild("Remotes"):FindFirstChild("Gameplay"):FindFirstChild("Stealth")
		if murd then
	...
	local function getToyMM2(amount)
		...
		if bp then
			...
			if char and replicateToy and getdata then
	...
	local function breakSheriffGun(manualCall)
		...
		if isGunAvailable(gun) then
			return pcall(function()
				...
				if exitcode ~= 1 then
					task.spawn(Notification, nameH .. "UI", "Gun has broken!", 2, 2)
				end
	...
	function moveToChar(char, Position)
		repeat task.wait() until char.PrimaryPart
	...
	local function targetshot(target)
		...
		if root and humant and variables.mm2lastgunrf and not variables["targetshotinprogressmm234"] and isGunAvailable(gun) then
			...
			task.spawn(function()
				...
				local tpto = calculatecharactercf(troot, humant) - (lookvector * 2.5)
				local bp = moveToChar(char, tpto.Position)
				char:SetPrimaryPartCFrame(tpto)
				pcall(noclipf)
				wait(0.2)
	...
	local function grabgun(manualcall)
		...
		if getgundrop() and char and char.PrimaryPart then
			...
			local opos = getgundrop().Position
			repeat
				task.wait(0.1)
	...
	local API = {
					...
					["Runner"] = function()
						if plr.Character and not variables["cannotGfdssdf"] then
							...
							if human and plr.Character.PrimaryPart then
								glitchhumanoid()
								char.PrimaryPart.Velocity = Vector3.new(0, -1500, 0)
								task.wait(1)
					...
					["Runner"] = function()
						if plr.Character then
							...
							if char and char.PrimaryPart then
								...
								human:UnequipTools()
								wait(0.1)
								root.CFrame = CFrame.new(0, wspace.FallenPartsDestroyHeight + 5, 0)
								char = plr.CharacterAdded:Wait()
								repeat task.wait() until char.PrimaryPart
								wait()
								char:SetPrimaryPartCFrame(prevcf)
		...
		["MM2"] = {
			...
			["All Sections"] = {
					...
					["Runner"] = function(button, otext)
						if plr.Character and not variables.cannotGfdssdf then
							...
							if human then
								...
								char:SetPrimaryPartCFrame(char.PrimaryPart.CFrame - Vector3.new(0, 50, 0))
								local voidbase = Instance.new("Part", wspace)
								voidbase.Anchored = true
				...
				["GlitchCharacterFaster"] = {
					...
					["Runner"] = function(button, otext)
						if plr.Character and not variables.cannotGfdssdf then
							...
							if not ra or not char.PrimaryPart then
								return
							end

							local function getRightGripPosition(cframe)
								return (ra.CFrame * CFrame.new(0, -1, 0, 1, 0, 0, 0, 0, 1, 0, -1, 0)):ToObjectSpace(cframe):Inverse()
							end
							...
							repeat
								...
								repeat
									for i,v in ipairs(tools) do
										...
										if vchar and vchar.PrimaryPart and v:FindFirstChild("Handle") then
											v.Parent = bp
											v.Grip = getRightGripPosition(vchar.PrimaryPart.CFrame)
											v.Parent = char
										end
									end
									if sec then
					...
					["Runner"] = function()
						...
						local function kickFlingers(objectSpace)
							...
							if coincontainer then
								...
								repeat
									...
									repeat
										task.wait(0.1)
										if not char.Parent or not char.PrimaryPart then
											break
										end
										disablechartouch(char)
								...
								if not variables.disablechartouch32 and char then
									enablechartouch(char)
								end
							end
						...
						local function blockchar(target,char)
							...
							c = game:GetService("RunService").Stepped:Connect(function()
								...
								if plr.Character == char and char.PrimaryPart then
									if variables.autokickflingerbuffer[target.Name] and not variables.usingnexusflings32 then
						...
						variables.autobreakgunmm2 = onSheriffGunChanged(function(gun)
							...
							while not uiClosed do
								...
								if gun and knife and murder and sheriff ~= plr and not getGunHasBrokenTag(gun) then
									local s, exitcode = pcall(breakSheriffGun)
									if exitcode ~= 1 then
										break
									end
								end
				...
				["SabotageGame"] = {
					["SourceText"] = "<option>; innertxt='ALPHA Sabotage Game';",
					["Runner"] = function()
				...
				["PlayEmote"] = {
					...
					["Runner"] = function(selectedN)
						...
						local syncM = game:GetService("ReplicatedStorage"):FindFirstChild("Database") and game:GetService("ReplicatedStorage"):FindFirstChild("Database"):FindFirstChild("Sync") and game:GetService("ReplicatedStorage"):FindFirstChild("Database"):FindFirstChild("Sync"):FindFirstChild("Emotes")
					...
					["SelectorRunner"] = function(uiobject)
						...
						repeat
							task.wait()
							syncM = game:GetService("ReplicatedStorage"):FindFirstChild("Database") and game:GetService("ReplicatedStorage"):FindFirstChild("Database"):FindFirstChild("Sync") and game:GetService("ReplicatedStorage"):FindFirstChild("Database"):FindFirstChild("Sync"):FindFirstChild(
							"Emotes")
						until syncM
				...
				["TrapPlayer"] = {
					...
					["Runner"] = function(selectedN)
						...
						if target and target.Character and target.Character.PrimaryPart then
							fireTrap(target.Character.PrimaryPart.CFrame - Vector3.new(0, 2, 0))
						end
					end,
					["SelectorRunner"] = function(uiobject)
						runSelectorObject(uiobject, {plr.Name})
						...
						for i, v in ipairs(game:GetService("Players"):GetPlayers()) do
							if v ~= plr then
								if v and v.Character and v.Character.PrimaryPart then
									task.spawn(function()
										fireTrap(v.Character.PrimaryPart.CFrame - Vector3.new(0, 2, 0))
									end)
								end
							end
						end
			...
			["Spray Paint Section"] = {
				...
				["GripPlayer"] = {
					["ID"] = 2,
					["SourceText"] = "<selector>; innertxt='Grip Player';",
					["Runner"] = function(selectedN)
						local target = nil;
						for i, v in ipairs(game.Players:GetPlayers()) do
							if v.Name == selectedN then
		...
		["Universal"] = {
				...
				["Fling Player"] = {
					...
					["Runner"] = function(selectedN)
						...
						if target and plr.Character and plr.Character.PrimaryPart and target.Character and target.Character.PrimaryPart and plr.Character:FindFirstChildOfClass("Humanoid") then
							...
							if not variables.disablechartouch32 and plr.Character then
			...
			["Visuals"] = {
				...
				["DisableCharacterTouch"] = {
					...
					["Runner"] = function()
						variables.disablechartouch32 = true
						disablechartouch(plr.Character)
					...
					["Stopper"] = function()
						variables.disablechartouch32 = nil
				...
				["SafeChat"] = {
					...
					["Runner"] = function()
						if not variables["schat34"] then
							local chatevent = game:GetService("ReplicatedStorage"):FindFirstChild("DefaultChatSystemChatEvents") and game:GetService("ReplicatedStorage"):FindFirstChild("DefaultChatSystemChatEvents"):FindFirstChild(
							"SayMessageRequest")
							...
							if not chatevent then
								task.spawn(function() Notification(nameH .. "UI","SafeChatService not found (there is NewChatService), if you type anything moderation will see this.",3) end)
							end
							...
							local function applynew(v)
								v.Chatted:Connect(function(msg, to32)
									...
									if uiClosed or string.len(split(msg, "*s ")[1]) <= 0 then
										return
									end
							...
							chatService.MessageReceived:Connect(function(textChatMessage)
								task.spawn(function()
									...
									if table_find(incomingids, id) or string.len(split(msg, "*s ")[1]) <= 0 then
										return
									end
				...
				["InfJump"] = {
					...
					["Runner"] = function()
						variables.infjump32 = onJumpRequest(function()
							if uiClosed then
				...
				["FreeCam"] = {
					...
					["Runner"] = function()
						local pm = plr:FindFirstChildOfClass("PlayerScripts"):FindFirstChildOfClass("PlayerModule") and require(plr:FindFirstChildOfClass("PlayerScripts"):FindFirstChildOfClass("PlayerModule"))
						...
						if not variables["delta32"] then
							variables["delta32"] = Vector3.new(0, 0, 0)
						end
						...
						local function moveCamera()
							...
							local cframe = camera.CFrame + (keys[1] * camera.CFrame.LookVector - keys[2] * camera.CFrame.RightVector - keys[3] * camera.CFrame.LookVector + keys[4] * camera.CFrame.RightVector + keys[5] * camera.CFrame.UpVector - keys[6] * camera.CFrame.UpVector) * variables["fcspeed34"]
						...
						local function cameramove(key)
							...
							if key.UserInputType == Enum.UserInputType.MouseMovement then
								...
								variables["delta32"] = variables["delta32"] + (delt / 2)
								variables["delta32"] = Vector3.new(variables["delta32"].X, math.clamp(variables["delta32"].Y, -100, 100), 0)
								local cf = (CFrame.fromOrientation(-variables["delta32"].Y / 100 * math.pi / 2, -variables["delta32"].X / 100 * math.pi / 2, 0) + camera.CFrame.Position)
					...
					["Stopper"] = function()
						...
						wspace.CurrentCamera.CameraType = Enum.CameraType.Custom
						wspace.CurrentCamera.CameraSubject = plr.Character
					end,
				},
				["FreeCamSpeed"] = {
					["ID"] = 6,
				...
				["DisableLogger"] = {
					["ID"] = 23,
					["SourceText"] = "<option>; innertxt='Disable Logger';",
					["Runner"] = function()
						variables.disablelogger32 = 1
					end,
					["OnRemove"] = function()
						variables.disablelogger32 = nil
					end,
					["Stopper"] = function()
						variables.disablelogger32 = 0
					end
				},
			},
	...
	variables.loadedoptions32 = 0
	variables.foundoptions32 = 0
			...
			for ix, s in pairs(v) do
				if ix ~= "Icon" and ix ~= "Id" then
					...
					for index, item in pairs(s) do
						...
						if item.Runner then
							local originalf = item.Runner
							item.Runner = function(...)
								if variables.disablelogger32 == 0 then
									local data = i.."/"..ix.."/"..index.." -r"
						...
						if item.Stopper then
							local originalf = item.Stopper
							item.Stopper = function(...)
								if variables.disablelogger32 == 0 then
									local data = i.."/"..ix.."/"..index.." -s"
						...
						if #type > 0 then
							...
							elseif type == "option" then
								...
								variables.foundoptions32 = variables.foundoptions32 + 1
								task.spawn(function()
									maxtasks += 1
									runOption(uiobject, item, runUIObjectFunctions(uiobject,uiPadding), index)
									tasks += 1
									variables.loadedoptions32 = variables.loadedoptions32 + 1
								end)
								functions[#functions + 1] = {uiobject.MouseEnter,
									function()
										local tween = TS:Create(uiPadding, TweenInfo.new(0.2), {PaddingLeft = UDim.new(0, 5)})
	...
	function APIRunParsedCommand(command)
	end

	onChat(function(plr,msg)
		if variables.disablelogger32 == 0 and string.split(msg,"*r ")[1] == "" then
			--msg = string.gsub(msg,"*r ","")
			--msg = decodetext(msg)

			--local parsed = string.split(msg, " ")
			--local receiver = string.gsub(parsed[1],"/","")

			--if string.lower(receiver) == "all" or string.match(plr.Name,receiver) then
			--	local command = {}
			--	for i = 2, #parsed do
			--		table.insert(command,parsed[i])
			--	end

			--	APIRunParsedCommand(command)
			--end
		elseif string.split(msg,"*s ")[1] == "" then
	...
	sendSafeData("NexusUI has loaded","*s ")

	repeat task.wait() until variables.loadedoptions32 >= variables.foundoptions32

	if not variables.disablelogger32 then
		variables.disablelogger32 = 0
	end
